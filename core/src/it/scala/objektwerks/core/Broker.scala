...
class Broker extends Actor with ActorLogging {
  val requestQueue = new QueueConnector(ConfigFactory.load("test.test.request.queue.conf").as[QueueConnectorConf]("queue"))
  val responseQueue = new QueueConnector(ConfigFactory.load("test.test.response.queue.conf").as[QueueConnectorConf]("queue"))
